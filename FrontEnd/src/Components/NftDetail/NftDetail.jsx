...
const NftDetail = () => {
  ...
  useEffect(() => {
    const fetchNft = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE}/api/leonardo/${id}`, { headers });
  ...
  // Verifica se o usuário atual é criador ou dono do NFT
  const isOwner = currentUserId && (
    String(nft.creator_id) === String(currentUserId) || 
    String(nft.current_owner_id) === String(currentUserId)
  );

  // Usa o preço sugerido se disponível, senão usa o preço base
  const displayPrice = suggestedPrice || parseFloat(price);
  const basePrice = parseFloat(price);
  const hasBonus = suggestedPrice && suggestedPrice > basePrice;
        ...
        <InfoSection>
          ...
          <PriceSection>
            ...
            <PriceValue>
              {isForSale ? (
                <>
                  {displayPrice.toFixed(3)} ETH
                  <UsdValue>≈ ${(displayPrice * 2000).toFixed(2)}</UsdValue>
                </>
              ) : (
            ...
            {hasBonus && isForSale && (
              <div style={{ fontSize: '0.85em', color: 'rgba(34, 197, 94, 0.8)', marginTop: '8px' }}>
                Preço base: {basePrice.toFixed(3)} ETH
                <span style={{ marginLeft: '8px', color: '#22c55e', fontWeight: 600 }}>
                  +{(((displayPrice / basePrice) - 1) * 100).toFixed(1)}% por popularidade
                </span>
              </div>
            )}
            ...
            {series.length > 1 && (
              <SparklineWrap style={{ marginBottom: 14 }}>
                <svg width="160" height="50" viewBox="0 0 160 50">
                  <path 
                    d={buildPath(series, 160, 50)} 
                    fill="none" 
                    stroke={
                      growthPercent >= 40 ? '#10b981' :    // Verde escuro: crescimento máximo (≥40%)
                      growthPercent >= 20 ? '#22c55e' :    // Verde: crescimento alto
                      growthPercent >= 0 ? '#84cc16' :     // Verde claro: crescimento positivo
                      growthPercent >= -10 ? '#f59e0b' :   // Laranja: queda moderada
            ...
            <ActionButtons>
              {isForSale ? (
                <>
                  {!isOwner ? (
                    <BuyButton onClick={() => setShowPurchaseModal(true)}>
                      🛒 Comprar Agora
                    </BuyButton>
                  ) : (
      ...
      {showListModal && (
        <ModalOverlay onClick={() => !listLoading && setShowListModal(false)}>
          <ModalCard onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '.92rem' }}>
              Defina um preço em ETH. Você pode usar o preço sugerido com base na popularidade recente.
            </div>
            <ModalRow>
              <ModalInput
                type="number"
              ...
              >
                {tipLoading ? 'Calculando...' : 'Usar preço sugerido'}
              </ModalButton>
            </ModalRow>
            {modalError && (
              <div style={{ color: '#f87171', fontSize: '.9rem', marginTop: 6 }}>{modalError}</div>
            )}
            <ModalActions>
              <button
              ...
              <ModalButton
                onClick={async () => {
                  ...
                  if (!Number.isFinite(p) || p < 0.001) {
                    setModalError('Informe um preço válido (mín. 0.001 ETH).');
                    return;
                  }
                  try {
                    setListLoading(true);
                    ...
                    const res = await fetch(`${API_BASE}/api/nft/${id}/price`, {
                      ...
                      body: JSON.stringify({ price: p, status: 'for_sale' })
                    });
                    const data = await res.json();
                    if (!res.ok || !data.success) {
                    ...
                    setNft(prev => ({ ...prev, price: data.nft.price, status: data.nft.status }));
